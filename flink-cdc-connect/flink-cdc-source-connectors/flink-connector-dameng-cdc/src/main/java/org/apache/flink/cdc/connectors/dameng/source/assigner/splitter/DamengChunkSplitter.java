...
package org.apache.flink.cdc.connectors.dameng.source.assigner.splitter;
...
import org.apache.flink.cdc.connectors.dameng.source.utils.DamengTypeUtils;
...
/**
 * The {@code ChunkSplitter} used to split Dameng table into a set of chunks for JDBC data source.
 */
@Internal
public class DamengChunkSplitter extends JdbcSourceChunkSplitter {

    public DamengChunkSplitter(
    ...
    public Object[] queryMinMax(JdbcConnection jdbc, TableId tableId, Column splitColumn)
            throws SQLException {
        // dameng query only use schema and table
        String quoteSchemaAndTable = DamengUtils.quoteSchemaAndTable(tableId);
    ...
    public Object queryNextChunkMax(
            ...
            throws SQLException {
        return DamengUtils.queryNextChunkMax(
    ...
    protected boolean isChunkEndLeMax(
        ...
        if (chunkEnd != null
                && chunkEnd.getClass().getName().contains("ROWID")
                && max != null
                && max.getClass().getName().contains("ROWID")) {
            String query =
    ...
    protected Column getSplitColumn(Table table, @Nullable String chunkKeyColumn) {
        // Use the ROWID column as the chunk key column by default for dameng cdc connector
