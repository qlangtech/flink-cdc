...
import static org.apache.flink.cdc.connectors.dameng.source.reader.fetch.DamengStreamFetchTask.RedoLogSplitReadTask;
import static org.apache.flink.cdc.connectors.dameng.source.utils.DamengUtils.buildSplitScanQuery;
import static org.apache.flink.cdc.connectors.dameng.source.utils.DamengUtils.readTableSplitDataStatement;

/** The task to work for fetching data of Dameng table snapshot split. */
public class DamengScanFetchTask extends AbstractScanFetchTask {

    public DamengScanFetchTask(SnapshotSplit split) {
        super(split);
    }
    ...
    protected void executeDataSnapshot(Context context) throws Exception {
        ...
        DamengSnapshotSplitReadTask snapshotSplitReadTask =
        ...
        SnapshotResult<DamengOffsetContext> snapshotResult =
    ...
    protected void executeBackfillTask(Context context, StreamSplit backfillStreamSplit)
        ...
        final DamengOffsetContext.Loader loader =
                new DamengOffsetContext.Loader(
                        ((DamengSourceFetchTaskContext) context).getDbzConnectorConfig(),
                        DamengConnectorConfig.ConnectorAdapter.LOG_MINER);
        final DamengOffsetContext damengOffsetContext =
    ...
    private RedoLogSplitReadTask createBackfillRedoLogReadTask(
            StreamSplit backfillRedoLogSplit, DamengSourceFetchTaskContext context) {
        ...
        return new RedoLogSplitReadTask(
                new DamengConnectorConfig(dezConf),
    ...
    public static class DamengSnapshotSplitReadTask
            extends AbstractSnapshotChangeEventSource<MapBackedPartition, DamengOffsetContext> {

        private static final Logger LOG =
                LoggerFactory.getLogger(DamengSnapshotSplitReadTask.class);
        ...
        private final EventDispatcher<MapBackedPartition, TableId> eventDispatcher;
        private final Clock clock;
        private final SnapshotSplit snapshotSplit;
        private final DamengOffsetContext offsetContext;
        private final SnapshotProgressListener<MapBackedPartition> snapshotProgressListener;
        ...
        @Override
        public SnapshotResult<DamengOffsetContext> execute(
                ChangeEventSourceContext context,
                MapBackedPartition partition,
                DamengOffsetContext previousOffset)
                throws InterruptedException {
            SnapshottingTask snapshottingTask = getSnapshottingTask(partition, previousOffset);
            final SnapshotContext<MapBackedPartition, DamengOffsetContext> ctx;
            try {
        ...
        protected SnapshotResult<DamengOffsetContext> doExecute(
                ...
                throws Exception {
            final DamengSnapshotContext ctx = (DamengSnapshotContext) snapshotContext;
        ...
        protected SnapshottingTask getSnapshottingTask(
                MapBackedPartition partition, DamengOffsetContext previousOffset) {
            return new SnapshottingTask(false, true);
        }

        @Override
        protected SnapshotContext<MapBackedPartition, DamengOffsetContext> prepare(
                MapBackedPartition partition) throws Exception {
            return new DamengSnapshotContext(partition);
        }

        private static class DamengSnapshotContext
                extends RelationalSnapshotChangeEventSource.RelationalSnapshotContext<
                        MapBackedPartition, DamengOffsetContext> {

            public DamengSnapshotContext(MapBackedPartition partition) throws SQLException {
                super(partition, "");
            }
        }

        private void createDataEvents(DamengSnapshotContext snapshotContext, TableId tableId)
                throws Exception {
            EventDispatcher.SnapshotReceiver<MapBackedPartition> snapshotReceiver =
        ...
        private void createDataEventsForTable(
                DamengSnapshotContext snapshotContext,
                EventDispatcher.SnapshotReceiver<MapBackedPartition> snapshotReceiver,
                Table table)
                throws InterruptedException {
        ...
        protected ChangeRecordEmitter<MapBackedPartition> getChangeRecordEmitter(
                SnapshotContext<MapBackedPartition, DamengOffsetContext> snapshotContext,
                TableId tableId,
                Object[] row) {
